...
#
# Install:
#   sudo cp install/lss-mgmt-backup.sh /usr/local/bin/lss-mgmt-backup.sh
#   sudo chmod 750 /usr/local/bin/lss-mgmt-backup.sh
#   sudo chown root:root /usr/local/bin/lss-mgmt-backup.sh
#   # Add to root's crontab:
#   sudo crontab -e
#     30 3 * * * /usr/local/bin/lss-mgmt-backup.sh >> /var/log/lss-mgmt-backup.log 2>&1
#
...
if [[ -z "$DB_PASS" ]]; then
    echo "[$(date -Is)] ERROR: LSS_BACKUP_PASS not set. Put it in /etc/default/lss-mgmt-backup (mode 600)." >&2
    exit 1
fi
