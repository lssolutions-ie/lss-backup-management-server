...
| 035 | `035_host_audit.sql` | Extends `audit_log.source` ENUM with `'host'` and adds `host_audit_state` for the journalctl cursor. Powers the host-audit worker that captures sshd logins, sudo invocations, and lss-backup-server.service lifecycle into the unified audit feed. |
